...
router.get('/test1', async (req, res) => {
	try {
		...
		// let data = await services.checkHandler(
		// 	'Enviopack',
		// 	'5079800000001918356',
		// 	'1 de agosto - 15:44 - El envío esta listo para ser despachado',
		// );
		let data = await db.Tracking.updateMany(
			{ completed: true, service: 'ViaCargo' },
			{ $unset: { destination: '' } },
			{ multi: true },
		).exec();

		// let trackings = await db.Tracking.find({ completed: true, service: 'ViaCargo' });
		// let operations = [];
		// for (let tracking of trackings) {
		// 	operations.push(tracking.updateOne({ _id: tracking.id }, { $set: { destination: 1 } }));
		// }
